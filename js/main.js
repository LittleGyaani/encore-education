;(function($){
	...
	if ($('.form-in-wrapp').size() > 0) {
		$("#ajax-contact-form").on('submit', function() {
			...
			$.ajax({
				type: "POST",
				url: "process/request/contact_process",
				data: str,
                dataType:'json',
                success:function(data){
                    ...
                    if(data.status=='OK'){
                        ...
                        swal({
                            title: data.message,
                            text: "Your request sent successfully! Ticket number : "+ data.ticketNumber,
                            icon: "success",
                            button: false,
							timer: 3000
                        });
                        ...
                        swal({
                            title: data.message,
                            text: "Unable to send your message. Please retry!",
                            icon: "error",
                            button: false,
                            timer: 1000
                        });
	...
	if ($('#map-canvas').size() > 0) {
		function initialize() {
			...
			var myLatlng = new google.maps.LatLng(20.2902869, 85.8449222);
			var mapOptions = {
				zoom: 13,
				scrollwheel: true,
				center: myLatlng,
            ...
            var icon = {
                ...
                scaledSize: new google.maps.Size(100, 100), // size of map marker icon
            };
			var map = new google.maps.Map(doc.getElementById('map-canvas'), mapOptions);
			var marker = new google.maps.Marker({
				position: myLatlng,
				map: map,
				title: 'Encore Education',
				draggable: true,
